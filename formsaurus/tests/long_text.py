...
from formsaurus.models import (Survey, Submission, LongTextAnswer)

User = get_user_model()

class LongTextTestCase(TestCase):
    def setUp(self):
    ...
    def test_required_long_text(self):
        ...
        response = self.client.post(reverse('formsaurus:question', args=[survey.id, question.id, submission.id]), {'answer': long_text})
        self.assertEqual(response.status_code, 302)
        answers = submission.answers()
        self.assertEqual(1, len(answers))
        answer = answers[0]
        self.assertTrue(isinstance(answer, LongTextAnswer))
        self.assertEqual(answer.long_text, long_text)
        # Update Answer
        ...
        answer = answers[0]
        self.assertTrue(isinstance(answer, LongTextAnswer))
        self.assertEqual(answer.long_text, updated_text)
        # Reject too long
